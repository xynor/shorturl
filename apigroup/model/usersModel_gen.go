...
var (
	cacheGozeroUsersIdPrefix  = "cache:gozero:users:id:"
	cacheGozeroUsersSeqPrefix = "cache:gozero:users:seq:"
)
...
type (
	usersModel interface {
		...
		FindOneBySeq(ctx context.Context, seq sql.NullString) (*Users, error)
	...
	Users struct {
		...
		Seq       sql.NullString `gorm:"column:seq"`
	}
)
...
func (m *defaultUsersModel) GetCacheKeys(data *Users) []string {
	...
	gozeroUsersIdKey := fmt.Sprintf("%s%v", cacheGozeroUsersIdPrefix, data.Id)
	gozeroUsersSeqKey := fmt.Sprintf("%s%v", cacheGozeroUsersSeqPrefix, data.Seq)
	cacheKeys := []string{
		gozeroUsersIdKey, gozeroUsersSeqKey,
	}
...
func (m *defaultUsersModel) FindOne(ctx context.Context, id uint64) (*Users, error) {
	gozeroUsersIdKey := fmt.Sprintf("%s%v", cacheGozeroUsersIdPrefix, id)
	var resp Users
	err := m.QueryCtx(ctx, &resp, gozeroUsersIdKey, func(conn *gorm.DB) error {
...
func (m *defaultUsersModel) FindOneBySeq(ctx context.Context, seq sql.NullString) (*Users, error) {
	gozeroUsersSeqKey := fmt.Sprintf("%s%v", cacheGozeroUsersSeqPrefix, seq)
	var resp Users
	err := m.QueryRowIndexCtx(ctx, &resp, gozeroUsersSeqKey, m.formatPrimary, func(conn *gorm.DB, v interface{}) (interface{}, error) {
		if err := conn.Model(&Users{}).Where("`seq` = ?", seq).Take(&resp).Error; err != nil {
			return nil, err
		}
		return resp.Id, nil
	}, m.queryPrimary)
...
func (m *defaultUsersModel) formatPrimary(primary interface{}) string {
	return fmt.Sprintf("%s%v", cacheGozeroUsersIdPrefix, primary)
}
